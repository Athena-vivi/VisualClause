  ...
  return (
    <>
      ...
      <button
        ...
        className="fixed bottom-8 right-8 z-40 w-14 h-14 bg-surface border border-accent-blue/50 rounded-full flex items-center justify-center shadow-metal hover:shadow-metal-intense transition-all duration-300 group"
      >
        {isOpen ? (
      ...
      {/* 聊天窗口 */}
      {isOpen && (
        <div className="fixed bottom-28 right-8 z-40 w-96 max-h-[600px] bg-surface border border-border shadow-glow rounded-sm flex flex-col animate-lattice-grow">
          {/* 头部 */}
          <div className="px-6 py-4 border-b border-border flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="text-text-primary text-sm font-medium">数字分身</h3>
                <p className="text-text-tertiary text-xs">在线 // INTJ模式</p>
              </div>
            </div>
          </div>
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-4">
            {messages.length === 0 && (
              <div className="text-center py-8">
                ...
                <p className="text-text-secondary text-sm">与我的数字分身对话</p>
                <p className="text-text-tertiary text-xs mt-2">崇尚逻辑 · 厌恶情绪</p>
              </div>
            )}
            ...
            {isTyping && (
              <div className="flex justify-start">
                <div className="bg-background border border-border px-4 py-2.5 rounded-tl-sm">
